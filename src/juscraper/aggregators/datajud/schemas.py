...
class InputContarProcessosDataJud(BaseModel):
    ...
    O conjunto de filtros (incluindo ``data_ajuizamento_*``,
    ``tipos_movimentacao``, ``movimentos_codigo``, ``orgao_julgador``,
    ``query`` override e os tipos extendidos de ``ano_ajuizamento``/
    ``classe``) bate com :class:`InputListarProcessosDataJud` — ver
    docstring la para detalhes semanticos.
    """
