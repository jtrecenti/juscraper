...
def _check_bot_challenge(resp: requests.Response, tribunal: str = "TRF5") -> None:
...
# ---------------------------------------------------------------------------
#
# PJe renders the movs table as a 15-row dataTable paged by a Richfaces 3
# inslider widget. Pages 2..N are fetched by POSTing the slider form back to
# ``listView.seam`` with ``AJAXREQUEST=<containerId>``. The auto-generated
# ``j_idNNN`` IDs come from the slider's ``onchange`` script in the detail.
...
class MovsPagination:
    ...
    form_id: str
    slider_input_name: str
    ajax_source_name: str
    container_id: str
    max_pages: int
    view_state: str
