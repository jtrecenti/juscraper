"""
Parse of cases from the TJAM Consulta de Julgados de Segundo Grau (CJSG).
TJAM uses the same eSAJ platform as TJSP, so the HTML structure is identical.
"""
import os
import glob
import re
...
logger = logging.getLogger("juscraper.tjam.cjsg_parse")
...
def cjsg_parse_manager(path: str) -> pd.DataFrame:
    ...
    for file in tqdm(arquivos, desc="Processando CJSG TJAM"):
        try:
