"""
Parse of cases from the TJAL Consulta de Julgados de Segundo Grau (CJSG).
TJAL uses the same eSAJ platform as TJSP, so the HTML structure is identical.
"""
import os
import glob
import re
...
logger = logging.getLogger("juscraper.tjal.cjsg_parse")
...
def cjsg_parse_manager(path: str) -> pd.DataFrame:
    ...
    for file in tqdm(arquivos, desc="Processando CJSG TJAL"):
        try:
