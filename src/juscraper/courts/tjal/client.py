...
from juscraper.utils.params import normalize_datas, normalize_paginas

from .cjsg_download import cjsg_download as _cjsg_download
from .cjsg_parse import cjsg_n_pags, cjsg_parse_manager

logger = logging.getLogger("juscraper.tjal")


class TJALScraper(BaseScraper):
    """Scraper for the Tribunal de Justica de Alagoas (TJAL).

    The TJAL uses the eSAJ platform (same as TJSP).
