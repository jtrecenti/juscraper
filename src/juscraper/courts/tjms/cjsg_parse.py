"""
Parse of cases from the TJMS Consulta de Julgados de Segundo Grau (CJSG).
TJMS uses the same eSAJ platform as TJSP, so the HTML structure is identical.
"""
import glob
import logging
import os
...
logger = logging.getLogger("juscraper.tjms.cjsg_parse")
...
def cjsg_parse_manager(path: str) -> pd.DataFrame:
    ...
    for file in tqdm(arquivos, desc="Processando CJSG TJMS"):
        try:
