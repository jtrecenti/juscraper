...
- TJRS CJSG: migrado do endpoint Solr legado (`/buscas/jurisprudencia/ajax.php`) para o novo frontend WordPress (`/novo/wp-admin/admin-ajax.php?action=busca_jurisprudencia`). O Solr legado fica intermitentemente retornando 503 (`no servers hosting shard: shard6`, `CoreContainer is either not initialized or shutting down`) e nao tem correcao do lado do cliente. O endpoint WordPress e um GET com parametros em query string e dates em `DD/MM/YYYY`, devolvendo um envelope JSON (`{data: {html, total, page, has_more, paginacao}}`) com os resultados pre-renderizados em HTML. `cjsg_parse_manager` reescrito para extrair processo/relator/orgao/classe/assunto/datas/ementa da HTML via BeautifulSoup. Algumas colunas internas do Solr (`documento_text`, `cod_*`, `_version_` etc.) deixam de existir porque nao sao expostas no frontend novo.
...
- Tres tribunais tem o teste de filtro de datas marcado como `xfail` estrito (lista `KNOWN_FILTRO_FAILURES` em `test_release_date_filter.py`): tjap (Tucujuris genuinamente nao expoe filtro por data — confirmado na engenharia reversa do HTML, JS e API do site: sem inputs, sem campos hidden, backend ignora silenciosamente qualquer variante de `dataInicial`/`dataInicio`/`dataJulgamento…`/`periodoInicio`), tjpb (backend filtra por campo interno diferente de `dt_ementa`), tjrj (endpoint legado ASP.NET devolve HTTP 500 ate sem filtros). Todas sao server-side — nao dependem do cliente. Cada fix deve remover o tribunal da lista — `strict=True` sinaliza quando o bug se resolve sozinho.
