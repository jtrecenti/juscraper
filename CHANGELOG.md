...
- Extra opcional `[tjmg]` em `pyproject.toml` declarando `txtcaptcha>=0.1.0` — dependencia ja exigida pelo `_solve_captcha` em `src/juscraper/courts/tjmg/download.py` mas que nunca havia sido formalizada nos metadados do projeto. Usuarios que vao raspar TJMG ao vivo agora instalam com `uv pip install -e ".[tjmg]"` (ou `pip install juscraper[tjmg]`); ambientes que nao mexem com TJMG seguem sem a dep. Como side-effect, `tests/tjmg/test_cjsg.py` (3 testes `@pytest.mark.integration`) ganha `pytest.importorskip("txtcaptcha", ...)` no top do arquivo — antes esses testes falhavam com `RuntimeError: TJMG requires txtcaptcha to decode its image captcha` em qualquer ambiente sem a dep; agora pulam de forma limpa com mensagem apontando para o extra. O `import` lazy + `RuntimeError` no codigo de producao continua intacto (mensagem clara mesmo se o usuario instalar so o `juscraper` base).
- `juscraper.core.parse_utils.clean_html(text, decode_entities=True)`: helper compartilhado de limpeza HTML — strip de tags + decodificacao de toda a tabela de entidades HTML5 via `html.unescape` (cobre nomeadas como `&Aacute;`/`&copy;`/`&hellip;` e numericas `&#NNN;`/`&#xHH;`). Expande o subset acute/grave/circ/etc. do `_clean_html` privado do TJAP. Com `decode_entities=False`, faz so o caminho minimalista (strip de tags + collapse whitespace) usado por TJRN/TJRO. Base para consolidacao do `_clean_html` privado nos 3 tribunais; migracao acontece nas Fases 1+ do refactor #194. Refs #201, #194.
- `juscraper.core.parse_utils.coerce_date_columns(df, cols)`: extrai o loop `pd.to_datetime(..., errors="coerce").dt.date` repetido em ~13 tribunais (TJAP, TJDFT, TJES, TJMT, TJPA, TJPE, TJRN, TJRO, TJRR, TJRS, TJSC, TJTO). Mutacao in-place + retorno do proprio DataFrame para encadeamento. Colunas ausentes sao ignoradas silenciosamente; `df` vazio retorna direto. Migracao acontece nas Fases 1+ do refactor #194. Refs #201, #194.
- `juscraper.core.http.HTTPScraper`: nova base entre `BaseScraper` e os scrapers concretos. Cria `self.session = requests.Session()` com User-Agent padrao, expoe o hook `_configure_session(session)` (mesmo contrato de `EsajSearchScraper._configure_session` para herdanca trivial na Fase 2 — #203), oferece `_request_with_retry(method, url, *, session=None, max_retries=3, base_backoff=2.0)` com backoff exponencial `base_backoff ** attempt` para 429/500/502/503/504 e respeito a `Retry-After` numerico (clamp em 0 quando o servidor retorna valor negativo; `max_retries < 1` levanta `ValueError`). Uma falha apos esgotar `max_retries` levanta a nova `juscraper.core.exceptions.RetryExhaustedError(status_code, attempts)`; 4xx nao-retryable sobe `requests.HTTPError`. **Resolve #185:** override por chamada `session=` e validado com `isinstance(session, requests.Session)` antes do request — valor invalido vira `TypeError` na fronteira da API publica em vez de erro opaco depois. `session=` fica deliberadamente fora dos schemas pydantic (decisao documentada em `CONTRIBUTING.md > Schemas pydantic > session= fica fora do schema`). Nao exportada em `juscraper.__init__` (uso interno; raspadores novos importam direto de `juscraper.core.http`). Tribunais existentes migram pelas Fases 1-4 do refactor #194 (#202-#205). Refs #201, #194, #185, #112.
- `juscraper.core.exceptions.HTTPSemanticError`: classe base abstrata para respostas HTTP-200 semanticamente erradas (captcha expirado, sessao invalida, formulario nao submetido). Subclasses concretas serao introduzidas pelos parsers conforme o 2o caso de cada tipo aparecer (Regra 2 do `CLAUDE.md`). Refs #201, #194.
- `juscraper.utils.cnj.format_cnj(numero, strict=False)`: novo modo lenient que retorna o input sem alterar quando `numero` e `None`, vazio, nao-string, ou nao tem 20 digitos apos limpeza. Default `strict=True` mantem o comportamento historico (levanta `ValueError`). Util para parsers cujos resultados misturam numeros formatados, brutos e ocasionalmente vazios (TJRN/TJRO/TJRR; migracao do `_format_cnj` privado para o helper compartilhado nas Fases 1+). Refs #201, #194.
- `tests/schemas/test_capture_drift.py` — lint AST anti-drift que parametriza por `tests/fixtures/capture/tj*.py` e valida que cada capture script importa pelo menos um simbolo absoluto de `juscraper.courts.<tribunal>.*` / `juscraper.courts._<familia>.*` ou o helper relativo `_util`. Operacionaliza o item 13 do checklist em `CONTRIBUTING.md` (payload builders publicos), pegando o caso comum de drift: capture script novo redefinindo `BASE_URL`/payload inline em vez de reusar do scraper. 17 capture scripts atuais passam (12 importam direto do `download.py` do tribunal, 5 da familia eSAJ delegam para `_util`).
...
- Schemas pydantic dos endpoints `cjsg`/`cjpg` ganham aperto de tipos para refletir os dominios fechados ja documentados nas docstrings. Valor invalido agora vira `ValidationError` na chamada publica em vez de ser passado ao backend e silenciosamente dropado/criticado opacamente. Suite generica de rejeicao de dominio em `tests/schemas/test_domain_rejection.py` cobre todos os apertos. Nao breaking: valores que eram validos antes continuam validos (incluindo `""` literal nos campos com novo default `None`). Refs #184. Apertos por categoria:
    - **Categoria A (`Literal[...]`):** TJMG `pesquisar_por`/`order_by`/`linhas_por_pagina`, TJGO `id_instancia`/`id_area`, TJPA `sort_order`, TJTO `tipo_documento`/`ordenacao` (cjsg+cjpg).
    - **Categoria B (`Field(ge=1)`):** TJBA `items_per_page`, TJDFT `quantidade_por_pagina`, TJES `per_page` (cjsg+cjpg), TJGO `qtde_itens_pagina`, TJMT `quantidade_por_pagina`. Apenas o lower bound — upper bound exige captura ao vivo do backend e fica como follow-up.
    - **Categoria C (`str = ""` -> `str | None = None`):** ~25 campos em TJGO, TJPB, TJRN, TJPI, TJRO, TJSC, TJRR, TJTO. Clients ganham coercao `or ""` (ou ternario `is not None else ""` para campos que aceitam `int | str`) antes da chamada ao downloader; `resolve_deprecated_alias` perde `sentinel=""` (alinhado com o novo default `None`).
    - **Categoria D (TODO follow-up):** campos com dominio fechado mas desconhecido (TJBA `ordenado_por`, TJES `ordenacao`, TJMT `tipo_consulta`, TJAP `orgao`/`votacao`, TJRJ `competencia`/`origem`, TJPA `sort_by`/`query_type`/`query_scope`, TJPB `id_origem`, TJGO `id_serventia_subtipo`) ganham comentario `# TODO` para captura ao vivo do backend.
- **TJPB `cjsg` / `cjsg_download` e TJTO `cjsg`/`cjpg` (+ `_download`)**: schemas pydantic `InputCJSGTJPB`, `InputCJSGTJTO` e `InputCJPGTJTO` wired nos metodos publicos via `apply_input_pipeline_search` em `cjsg_download`/`cjpg_download` (padrao #183). Kwargs desconhecidos passam a levantar `TypeError` em vez de serem silenciosamente dropados; filtros com formato invalido viram `ValidationError`. Datas (`data_julgamento_inicio`/`fim`) agora aceitam as quatro variacoes canonicas de string (`DD/MM/YYYY`, `DD-MM-YYYY`, `YYYY-MM-DD`, `YYYY/MM/DD`) e tambem `datetime.date`/`datetime.datetime` — o backend do TJPB espera `YYYY-MM-DD`, o do TJTO espera `DD/MM/YYYY` (declarados em `BACKEND_DATE_FORMAT` em cada schema). TJPB `cjsg` mantem o post-filter local sobre `data_julgamento` (refs #195). Docstrings reescritas no padrao `**kwargs` da familia eSAJ. Fecha o gap apontado pela entrada anterior do CHANGELOG ("quarta leva de #165") que listou TJPB e TJTO como wired sem o wiring de fato ter entrado. Refs #93, #165, #195.
...
- **BREAKING:** `TJGOScraper.cjsg` e `cjsg_download` agora rejeitam `data_julgamento_inicio` / `data_julgamento_fim` com `TypeError` (via schema `InputCJSGTJGO` com `extra="forbid"`) em vez de emitir `UserWarning` (`warn_unsupported`) e seguir a busca sem o filtro. O backend Projudi do TJGO so expoe filtro de data de publicacao; codigo que passava `data_julgamento_*` recebia resultado nao-filtrado silenciosamente — agora falha alto e claro em ambos os pontos de entrada publicos. Migracao: usar `data_publicacao_inicio` / `data_publicacao_fim` (canonico para o TJGO) ou remover o argumento. Refs #93, #165, #183.
- TJMT `cjsg`: filtros `data_publicacao_inicio` / `data_publicacao_fim` agora levantam `TypeError` em vez de serem silenciosamente descartados. O backend Hellsgate expoe um unico intervalo de datas (`filtro.periodoDataDe`/`filtro.periodoDataAte`) aplicado a data de julgamento, exposto canonicamente via `data_julgamento_*`. `InputCJSGTJMT` deixa de herdar `DataPublicacaoMixin`, e os parametros `data_publicacao_*` saem da assinatura publica de `cjsg`/`cjsg_download`. Refs #165, #173.
...
- Anotacoes de tipo modernizadas para PEP 604/585 em `src/` e `tests/`: `Optional[X]` -> `X | None`, `Union[X, Y]` -> `X | Y`, `List[X]`/`Dict[X, Y]`/`Tuple[X, ...]` -> `list[X]`/`dict[X, Y]`/`tuple[X, ...]`. `Literal`, `Any`, `Callable`, `ClassVar` continuam vindo de `typing` (sem equivalente nativo). Adicionado `ruff` em dev deps + hook em `.pre-commit-config.yaml` com regras `UP006`/`UP007`/`UP035`/`UP045` para travar regressao. Sem mudanca de comportamento. Refs #149.
...
- TJRJ `cjsg`: o POST inicial agora envia o hidden `ctl00$ContentPlaceHolder1$hfListaPalavrasBloqueadas`, mimetizando o form ASPX real. O backend ASPX comecou a 500 transientemente em 2026-04-30 quando esse campo era omitido (issue surfaceada pelo PR de contratos 1C-b). Junto com isso, `InputCJSGTJRJ` foi wired no metodo publico via `apply_input_pipeline_search` — kwargs desconhecidos (incluindo `data_julgamento_*` e `data_publicacao_*`, que o backend nao expoe — granularidade so anual) passam a levantar `TypeError` em vez de serem silenciosamente dropados pelo antigo `warn_unsupported`. Contratos offline `tests/tjrj/test_cjsg_contract.py` + `test_cjsg_filters_contract.py` capturam o regressao. Refs #93, #143.
- TJPB `cjsg`: post-filter local sobre `data_julgamento` agora roda mesmo quando so uma das datas (`data_julgamento_inicio` ou `data_julgamento_fim`) e informada — usa `date.min`/`date.max` como bound aberto no lado ausente. Antes a guard exigia ambas as datas e linhas com `dt_ementa` fora da janela voltavam silenciosamente. O helper local de coercao tambem foi reescrito sobre `coerce_brazilian_date`, garantindo que o post-filter respeite todos os formatos aceitos pelo schema (`DD/MM/YYYY`, `DD-MM-YYYY`, `YYYY-MM-DD`, `YYYY/MM/DD`, `date`, `datetime`) — antes o `_parse_br` so reconhecia `DD/MM/YYYY` e silenciosamente pulava o filtro para os demais formatos. Refs #195.
- TJGO, TJRR, TJMG `cjsg_download`: kwargs desconhecidos passam a levantar `TypeError` em vez de serem silenciosamente dropados. O `apply_input_pipeline_search` desses tribunais foi plugado em `cjsg` (top-level) na implementacao inicial deste PR, deixando `cjsg_download` (chamada lower-level publica) aceitando filtros invalidos sem erro nem warning — regressao do objetivo da #165. Pipeline relocalizado para `cjsg_download` (padrao TJDFT/TJES/TJBA), `cjsg` virou wrapper trivial. Refs #183.
- TJPR, TJPI, TJRN, TJRO, TJSC `cjsg_download`: kwargs desconhecidos passam a levantar `TypeError` em vez de serem silenciosamente dropados. O `apply_input_pipeline_search` desses tribunais estava plugado em `cjsg` (top-level), o que deixava `cjsg_download` (chamada lower-level publica) aceitando filtros invalidos sem erro nem warning — regressao do objetivo da #165. Pipeline relocalizado para `cjsg_download` (padrao TJDFT/TJES/TJBA), `cjsg` virou wrapper trivial. Inclui caso TJPI: `data_publicacao_*` agora cai em `TypeError` tambem em `cjsg_download` (ja caia em `cjsg`). TJPA tinha duplicacao do pipeline em ambos os metodos — `cjsg` enxugado para wrapper trivial, eliminando a redundancia. Refs #183, #165.
