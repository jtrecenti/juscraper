...
- TJPI `cjsg`: novo teste `test_cjsg_data_inicio_kwarg_is_silently_dropped` trava o comportamento atual (bug latente) — TJPI nao tem filtro de data no backend e `TJPIScraper.cjsg` nao repassa `**kwargs` para `cjsg_download`, entao hoje passar `data_inicio`/`data_fim` e silenciosamente ignorado (sem `TypeError`, sem `DeprecationWarning`, sem `UserWarning`). A conversao para o par `DeprecationWarning` + `warn_unsupported` prescrito pela regra 6a exige wiring de `normalize_datas` no metodo publico e entrara junto com a refatoracao #84 da familia 1C; quando isso acontecer, o teste deve ser substituido pela asserção do par de warnings. Follow-up identificado no review do PR #123.
