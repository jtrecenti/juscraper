...
- TJDFT, TJGO, TJRJ, TJPA e TJPE: `cjsg` passa a usar `core.http.HTTPScraper` (backoff exponencial centralizado para 429/5xx, respeito a `Retry-After`) e — onde aplicavel — os helpers `clean_html` (TJGO) e `coerce_date_columns(..., date_format="%Y-%m-%d")` (TJDFT, TJPA). Mesmas colunas e mesmo payload do request. Mudancas observaveis herdadas da infra centralizada: (a) esgotar `max_retries` em status retryable agora levanta `RetryExhaustedError` em vez de propagar a ultima `requests.HTTPError`; (b) em TJPA, o retry exponencial dentro de `_fetch_page` foi removido — agora so 429/5xx fazem retry (antes qualquer `RequestException`/`ValueError` retentava). **Mudanca observavel em TJGO**: o campo `texto` agora separa tokens entre tags adjacentes — `<p>foo</p><b>bar</b>` vira `"foo bar"` em vez de `"foobar"` (efeito do `clean_html` substituir tags por espaco em vez de string vazia, mesmo trade-off documentado em TJMT no batch anterior). Conferido nas 4 amostras versionadas: o output passa a colar parentesis e palavras corretamente em alguns trechos (`abrirArquivo(...)` deixa de virar `abrirArquivo`) e nao quebra o tipo de nenhum campo. **Breaking change em `TJPEScraper.cjsg`/`cjsg_download`**: o parametro `session: requests.Session | None = None` foi removido; o override de session passa pelo `HTTPScraper._request_with_retry` (consumido via `self.session`). Refs #194, #202.
