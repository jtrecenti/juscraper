...
- **BREAKING:** `TJGOScraper.cjsg` agora rejeita `data_julgamento_inicio` / `data_julgamento_fim` com `TypeError` (via schema `InputCJSGTJGO` com `extra="forbid"`) em vez de emitir `UserWarning` (`warn_unsupported`) e seguir a busca sem o filtro. O backend Projudi do TJGO so expoe filtro de data de publicacao; codigo que passava `data_julgamento_*` recebia resultado nao-filtrado silenciosamente — agora falha alto e claro. Migracao: usar `data_publicacao_inicio` / `data_publicacao_fim` (canonico para o TJGO) ou remover o argumento. Refs #93, #165.
