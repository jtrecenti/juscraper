...
- `juscraper.utils.params.apply_input_pipeline_search(schema_cls, method_name, *, pesquisa, paginas, kwargs, max_dias=None, origem=None, **canonical_filters)` — pipeline canonico de validacao de inputs cjsg/cjpg em uma chamada: `normalize_paginas` -> `normalize_datas` -> `pop_normalize_aliases` -> `validate_intervalo_datas` (julgamento + publicacao) -> `schema_cls(...)` -> `raise_on_extra_kwargs` (ValidationError de `extra_forbidden` vira TypeError amigavel). Reusado por `_esaj/base.py:cjsg_download` e `tjsp/client.py:cjpg_download` (eSAJ passa `max_dias=366, origem="O eSAJ"`); pronto para os 5 tribunais 1C-a (TJRN, TJPA, TJRO, TJSC, TJPI) ao serem wired pelo refactor #84. Por default, `max_dias=None` desabilita a checagem de janela — alinhado com a auditoria empirica #128, que mostrou que apenas a familia eSAJ tem limite de 366 dias. Quando `max_dias` e setado sem `origem`, fallback para `"O backend"` para nao vazar "eSAJ" para callers de outras familias. Substitui ~15 linhas de boilerplate duplicado por chamada. Refs #84, #93, #128.
- `juscraper.utils.params.raise_on_extra_kwargs(exc, method)` — promovido de `_esaj/base.py:_raise_on_extra` para a API publica de utils. Converte `pydantic.ValidationError` com `extra_forbidden` puro em `TypeError` amigavel; outros erros de validacao propagam. Refs #84, #93.
- Contrato offline para `DatajudScraper.listar_processos` em `tests/datajud/test_listar_processos_contract.py` (typical multi-pagina com `search_after`+`OrderedRegistry`, single-page, no-results, filtro CNJ com alias inferido por `id_justica`+`id_tribunal`, filtro tribunal explicito com `tamanho_pagina` custom, `mostrar_movs=True`) e `tests/datajud/test_listar_processos_filters_contract.py` (todos os filtros simultaneos com `json_params_matcher` confirmando `must_conditions` na ordem canonica + range dual de `dataAjuizamento`, CNJ invalido sem chamada a API, kwarg desconhecido via `raises((ValidationError, TypeError))`). Samples crus em `tests/datajud/samples/listar_processos/` capturados via `python -m tests.fixtures.capture.datajud` contra a API publica do CNJ. O agregador nao tem aliases deprecados (regra 6a do checklist N/A). Wiring de `InputListarProcessosDataJud` segue como follow-up dedicado (regra do projeto: contrato e wiring nunca no mesmo PR). Drift assumido entre o capture script e `aggregators/datajud/client.py:_listar_processos_por_alias` documentado in-source — segundo follow-up extrai `build_listar_processos_payload` em `download.py` (regra 12 do CLAUDE.md). Substitui o paragrafo "DataJud" do Sub-PR 1D da #104. Refs #104, #113, #140.
- Contratos de teste offline para TJPR (`cjsg`), primeira fatia do sub-PR 1C-c. Contrato cobre o fluxo `GET home → POST pesquisa.do (paginação) → GET actionType=exibirTextoCompleto` (chamado uma vez por linha "Leia mais..."), com 3 cenários (`typical_com_paginacao`, `single_page`, `no_results`) + propagação de filtros (`data_julgamento_*`, `data_publicacao_*`) + aliases deprecados `query`/`termo`/`data_inicio`/`data_fim`. Samples reais capturados em `tests/tjpr/samples/cjsg/` via `tests/fixtures/capture/tjpr.py`. TJPE/TJRR/TJMG ficam para sub-PRs subsequentes — TJPE adiado por indisponibilidade do backend no momento da captura. Refs #104, #113, #116, #122.
- `tests._helpers.urlencoded_body_subset_matcher` e `tests._helpers.query_param_subset_matcher`: matchers do `responses` que validam apenas um subset de campos (body urlencoded e query params, respectivamente), úteis quando o scraper envia tokens dinâmicos (CSRF, ViewState, crypto) que o contrato não deve fixar. Promovidos a partir de duplicação byte-a-byte entre `tests/tjpr/test_cjsg_contract.py` e `tests/tjpr/test_cjsg_filters_contract.py`; ficam disponíveis para os próximos sub-PRs 1C-c (TJPE/TJRR/TJMG).
...
- `DatajudScraper.listar_processos` aceita `paginas` em todas as 4 formas do contrato unico (`int | list[int] | range | None`), alinhado com `SearchBase` e os demais raspadores. `int` e `list` sao normalizados em runtime via `juscraper.utils.params.normalize_paginas`; lista esparsa (`paginas=[3, 5]`) vira `range(min, max+1)` antes da iteracao porque o cursor `search_after` da API DataJud e forwards-only — o usuario recebe o DataFrame contiguo das paginas 3, 4 e 5. `paginas` e extraido para `juscraper.schemas.PaginasMixin` e `SearchBase` passa a herdar dele em vez de declarar o campo diretamente; agregadores sem `pesquisa` (caso DataJud) compoem o mixin direto. Os 4 entries em `tests/schemas/test_paginas_acceptance.py::XFAIL_CASES` sao removidos. Refs #118.
- Schema pydantic `InputListarProcessosDataJud` wired no metodo publico `DatajudScraper.listar_processos`: kwargs desconhecidos passam a levantar `TypeError` com mensagem `DatajudScraper.listar_processos() got unexpected keyword argument(s): '<nome>'` em vez do `TypeError` nativo do Python (mais opaco). Filtros com formato invalido (ex.: `ano_ajuizamento` nao-int) viram `ValidationError` direto do pydantic. Schema registrado como `DatajudScraper.INPUT_LISTAR_PROCESSOS` para `tests/schemas/test_signature_parity` reconhecer o wiring (mesmo padrao `INPUT_CJSG` da familia eSAJ). Refs #93, #152.
- `juscraper.aggregators.datajud.download.build_listar_processos_payload` extraido de `client.py:_listar_processos_por_alias` e reusado pelo capture script `tests/fixtures/capture/datajud.py`. Single source of truth para o body Elasticsearch (regra 12 do CLAUDE.md): client em producao e capture/contratos compartilham o mesmo builder, eliminando o drift documentado em #140. Refactor sem mudanca de comportamento (`json_params_matcher` dos contratos validam byte-a-byte). Refs #153.
- `juscraper.utils.params.validate_intervalo_datas` aceita `max_dias=None` para desabilitar a checagem de janela mantendo a validacao de formato e ordem (`inicio <= fim`). Permite `apply_input_pipeline_search` ser reusado por tribunais sem limite documentado de janela (refs #128) — apenas eSAJ continua passando `max_dias=366` explicitamente.
