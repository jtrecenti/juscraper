...
- Schemas pydantic dos endpoints `cjsg`/`cjpg` ganham aperto de tipos para refletir os dominios fechados ja documentados nas docstrings: campos com dominio discreto conhecido viram `Literal[...]` (TJMG `pesquisar_por`/`order_by`/`linhas_por_pagina`, TJGO `id_instancia`/`id_area`, TJPA `sort_order`, TJTO `tipo_documento`/`ordenacao`); paginacao numerica ganha `Field(ge=1, le=100)` em TJBA `items_per_page`, TJDFT `quantidade_por_pagina`, TJES `per_page` (cjsg+cjpg), TJGO `qtde_itens_pagina`, TJMT `quantidade_por_pagina`; defaults sentinela `str = ""` viram `str | None = None` em ~25 campos (TJGO, TJPB, TJRN, TJPI, TJRO, TJSC, TJRR, TJTO). Valor invalido agora vira `TypeError`/`ValidationError` na chamada publica em vez de ser passado ao backend e silenciosamente dropado/criticado opacamente. Nao breaking: valores que eram validos antes continuam validos (incluindo `""` literal nos campos com novo default `None`). Suite generica de rejeicao de dominio em `tests/schemas/test_domain_rejection.py` cobre todos os apertos. Campos com dominio fechado mas desconhecido (TJBA `ordenado_por`, TJES `ordenacao`, TJMT `tipo_consulta`, TJAP `orgao`/`votacao`, TJRJ `competencia`/`origem`, TJPA `sort_by`/`query_type`/`query_scope`, TJPB `id_origem`, TJGO `id_serventia_subtipo`) ganham comentario `# TODO` para captura ao vivo do backend em PR follow-up. Refs #184.
