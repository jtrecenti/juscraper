...
- `DatajudScraper.{contar,listar}_processos`: `ano_ajuizamento` agora aceita `int` (mantido), `tuple[int, int]` (range inclusivo, ex.: `(2020, 2024)`) ou `list[int]` (anos discretos, ex.: `[2020, 2022, 2024]`); `classe` agora aceita `str` (mantido) ou `list[str]` (varios codigos OR-ed via `terms`). Cada ano vira um par de `range` (ISO + compacto) OR-ed via `should` no body Elasticsearch; helpers `_build_ano_ajuizamento_clause` e `_build_classe_clause` em `aggregators/datajud/download.py` sao reusados pelos dois builders (`build_listar_processos_payload`, `build_contar_processos_payload`). Tuple invertida (`(2024, 2020)`) e ordenada; `(2023, 2023)` ≡ `2023` (mesmo payload); list vazia ≡ `None`. Sem breaking change. `contar_processos` tambem ganha paridade com `listar_processos` no novo escopo de #176 (`data_ajuizamento_inicio`/`fim`, `tipos_movimentacao`, `movimentos_codigo`, `orgao_julgador`, `query` override). Refs #178.
