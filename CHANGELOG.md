...
- TJRR CJSG: descoberta dinamica dos nomes dos campos JSF. Os IDs auto-gerados pelo JSF (`menuinicial:j_idt28` para pesquisa, `j_idt30` para submit etc.) mudam cada vez que o tribunal adiciona/reordena componentes no servidor; o scraper ficou silenciosamente retornando zero resultados depois da ultima renumeracao. `_get_form_fields` agora resolve o input de pesquisa via `id=consultaAtual` e descobre o botao submit pelo primeiro `<button>` com `name` `menuinicial:j_idt…` que nao comeca com `menuinicial:btn_`. O parametro `relator` (texto) foi descontinuado — o formulario agora expoe apenas `relatorList*` (checkboxes por categoria).
