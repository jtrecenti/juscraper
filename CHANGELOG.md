...
- `juscraper.core.parse_utils.clean_html(text, decode_entities=True)`: helper compartilhado de limpeza HTML — versao completa do TJAP (tags + entidades nomeadas via `html.unescape` + entidades numericas `&#NNN;`/`&#xHH;`). Com `decode_entities=False`, faz so o caminho minimalista (strip de tags + collapse whitespace) usado por TJRN/TJRO. Base para consolidacao do `_clean_html` privado nos 3 tribunais; migracao acontece nas Fases 1+ do refactor #194. Refs #201, #194.
...
- `juscraper.core.http.HTTPScraper`: nova base entre `BaseScraper` e os scrapers concretos. Cria `self.session = requests.Session()` com User-Agent padrao, expoe o hook `_configure_session(session)` (mesmo contrato de `EsajSearchScraper._configure_session` para herdanca trivial na Fase 2 — #203), oferece `_request_with_retry(method, url, *, session=None, max_retries=3, base_backoff=2.0)` com backoff exponencial `base_backoff ** attempt` para 429/500/502/503/504 e respeito a `Retry-After` numerico. Uma falha apos esgotar `max_retries` levanta a nova `juscraper.core.exceptions.RetryExhaustedError(status_code, attempts)`; 4xx nao-retryable sobe `requests.HTTPError`. **Resolve #185:** override por chamada `session=` e validado com `isinstance(session, requests.Session)` antes do request — valor invalido vira `TypeError` na fronteira da API publica em vez de erro opaco depois. `session=` fica deliberadamente fora dos schemas pydantic (decisao documentada em `CONTRIBUTING.md > Schemas pydantic > session= fica fora do schema`). Nao exportada em `juscraper.__init__` (uso interno; raspadores novos importam direto de `juscraper.core.http`). Tribunais existentes migram pelas Fases 1-4 do refactor #194 (#202-#205). Refs #201, #194, #185, #112.
