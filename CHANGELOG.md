...
- `juscraper.utils.params.apply_input_pipeline_cjsg(schema_cls, method_name, *, pesquisa, paginas, kwargs, **canonical_filters)` — pipeline canonico de validacao de inputs cjsg/cjpg em uma chamada: `normalize_paginas` -> `normalize_datas` -> `pop_normalize_aliases` -> `validate_intervalo_datas` (julgamento + publicacao) -> `schema_cls(...)` -> `raise_on_extra_kwargs` (ValidationError de `extra_forbidden` vira TypeError amigavel). Reusado por `_esaj/base.py:cjsg_download` e `tjsp/client.py:cjpg_download`; pronto para os 5 tribunais 1C-a (TJRN, TJPA, TJRO, TJSC, TJPI) ao serem wired pelo refactor #84. Substitui ~15 linhas de boilerplate duplicado por chamada. Refs #84, #93.
