"""Filter-propagation contract for TJAM cjsg (refs #84, #104 comment)."""
import pandas as pd
import pytest
import responses
...
def test_cjsg_all_filters_land_in_post_body(tmp_path, mocker):
    ...
    responses.add(
        ...
        body=load_sample_bytes("tjam", "cjsg/no_results.html"),
        status=200,
...
def test_cjsg_unknown_kwarg_raises(tmp_path):
    scraper = jus.scraper("tjam", download_path=str(tmp_path))
