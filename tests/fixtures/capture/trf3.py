"""Capture cpopg samples for TRF3 against the live PJe deployment.

Run from repo root::

    python -m tests.fixtures.capture.trf3

Writes ``form_initial.html``, ``search_one_result.html``,
``search_no_results.html`` and ``detail_normal.html`` to
``tests/trf3/samples/cpopg/`` for the offline contract tests.

Goes through the *real* scraper helpers in
:mod:`juscraper.courts.trf3.download` — using the same payload-builder and
...
# Single-result CNJ taken from data/amostra_jf_primeiro_grau.csv — an active
# JEF process (São José do Rio Preto / SP) likely to keep returning a hit.
FOUND_CNJ = "50059460920254036324"

# Synthetic CNJ shaped like a TRF3 process so the form mask validation passes;
# backend returns an empty Ajax fragment with no ca token.
MISSING_CNJ = "00000000020994030000"


def main() -> None:
    """Capture cpopg samples for TRF3."""
    dest = REPO_ROOT / "tests" / "trf3" / "samples" / "cpopg"
    ...
    if not ca:
        raise RuntimeError(
            f"[trf3] sample capture failed: no ca token for {FOUND_CNJ!r} — "
            "process may have been archived or sigilo-protected."
        )
    print(f"[trf3]   found ca token: {ca}")
    ...
    print(f"[trf3] ALL samples written to {dest}")
