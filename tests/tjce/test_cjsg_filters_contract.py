"""Filter-propagation contract for TJCE cjsg (refs #84, #104 comment)."""
import pandas as pd
import pytest
import responses
...
def test_cjsg_all_filters_land_in_post_body(tmp_path, mocker):
    ...
    responses.add(
        ...
        body=load_sample_bytes("tjce", "cjsg/no_results.html"),
        status=200,
...
def test_cjsg_unknown_kwarg_raises(tmp_path):
    scraper = jus.scraper("tjce", download_path=str(tmp_path))
